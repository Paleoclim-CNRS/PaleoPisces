...
from pisces_modules.tools.visuals import color, str_infos

from pisces_modules.tools.utils import (path_checker,
                                        find_in_str,
                                        find_replace_in_file,
                                        get_outputs)

from pisces_modules.tools.paths import (user_path,
                                        WORK_PATH,
                                        SOURCES_PATH,
                                        JB_PATH,
                                        IGCM_PATH)

from pisces_modules.user_input.sim_path import (SIMULATION_TYPE,
                                                        SIM_OUTPUT_PATH,
                                                        FILE_PREFIX,
                                                        MASK_PATH,
                                                        MASK_FILE,
...
from pisces_modules.user_input.config_card import (JOB_NAME,
                                                   DATE_BEGIN,
                                                   DATE_END,
                                                   SPACE_NAME)
...
class PaleoPiscesInstaller():
    ...
    def cp_source_files(self):
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'arch-X64_IRENE.fcm'),
            os.path.join(
                self.pisces_path, 'modipsl', 'modeles', 'NEMOGCM', 'ARCH')
        )
        # cp cpp_ORCA2_OFF_PISCES.fcm file
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'cpp_ORCA2_OFF_PISCES.fcm'),
            os.path.join(
        ...
        shutil.copy2(os.path.join(SOURCES_PATH, 'iodef.xml'),
            os.path.join(self.pisces_path, 'modipsl', 'modeles', 'NEMOGCM',
                'CONFIG', 'ORCA2_OFF_PISCES', 'EXP00'))

        shutil.copy2(os.path.join(SOURCES_PATH, 'field_def.xml'),
                     os.path.join(self.pisces_path, 'modipsl', 'modeles', 'NEMOGCM',
                'CONFIG', 'ORCA2_OFF_PISCES', 'SHARED'))

        shutil.copy2(os.path.join(SOURCES_PATH, 'namelist_pisces_ref'),
                     os.path.join(self.pisces_path, 'modipsl', 'modeles', 'NEMOGCM',
                'CONFIG', 'ORCA2_OFF_PISCES', 'SHARED'))
...
class PaleoPiscesConfigurator():
    ...
    def mod_exp_files(self):
        ...
        if SIMULATION_TYPE == 'ia':
            shutil.copy2(
                os.path.join(
                    SOURCES_PATH, 'pisces.driver'),
                os.path.join(
                    self.pisces_path, 'modipsl', 'config', 'NEMO_v6', 'GENERAL', 'DRIVER')
            )

        if SIMULATION_TYPE == 'clim':
            shutil.copy2(
                os.path.join(
                    SOURCES_PATH, 'namelist_offline_clim_cfg_clim'),
                os.path.join(
                    self.pisces_path, 'modipsl', 'config', 'NEMO_v6',
                    'GENERAL', 'PARAM', 'NAMELIST', 'ORCA2', 'namelist_offline_clim_cfg'
                )
            )
        elif SIMULATION_TYPE == 'ia':
            shutil.copy2(
                os.path.join(
                    SOURCES_PATH, 'namelist_offline_clim_cfg_ia'),
                os.path.join(
                    self.pisces_path, 'modipsl', 'config', 'NEMO_v6',
                    'GENERAL', 'PARAM', 'NAMELIST', 'ORCA2', 'namelist_offline_clim_cfg'
                )
    ...
    def cp_bc_files(self):
        ...
        if SIMULATION_TYPE == 'clim':
            ...
            # SYMLINKS
            os.symlink(
                os.path.join(SOURCES_PATH, 'coordinates_paleorca2_yd.nc'),
            ...
            print(f'\n {str_infos["warning"]} Simulation type is "ia", few more steps need to be '
                  'executed to unpack the coupled simulation files with\n'
                  f' {color["YELOW"]}/ccc/work/cont003/gen2212/p25ladan/'
                  f'BC_PISCES_OFFLINE/Extract.ksh{color["END"]}')
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'UpdateVariable_sic.ksh'),
            os.path.join(self.bc_path)
        )
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'UpdateVariable_woce.ksh'),
            os.path.join(self.bc_path)
        )
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'ScaleNutrients.ksh'),
            os.path.join(self.bc_path)
        )
    ...
    def weight_tool(self):
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'namelist_r144x143_paleorca2_bilin'),
            os.path.join(self.bc_path, 'WEIGHTS', 'data')
        )
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'namelist_r144x96_paleorca2_bilin'),
            os.path.join(self.bc_path, 'WEIGHTS', 'data')
        )
    ...
    def coastal_mask(self):
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'create_coastline.f90'),
            os.path.join(self.bc_path)
        )
...
class PaleoPiscesInitializer():
    ...
    def config_cards(self):
        ...
        if SIMULATION_TYPE == 'clim':
            shutil.copy2(
                os.path.join(SOURCES_PATH, 'config.card_clim'),
                os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6', 'config.card')
            )
        elif SIMULATION_TYPE == 'ia':
            shutil.copy2(
                os.path.join(SOURCES_PATH, 'config.card_ia'),
                os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6', 'config.card')
            )
    ...
    def pisces_card(self):
        ...
        if SIMULATION_TYPE =='clim':
            shutil.copy2(
                    os.path.join(SOURCES_PATH, 'pisces.card'),
            ...
            print(f'{str_infos["warning"]} modification of pisces.card'
                   'is not handled with "ia" simulation type')
    ...
    def modif_param_files(self):
        ...
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'file_def_nemo-pisces_offline.xml'),
            os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6', self.job_name, 'PARAM', 'XML')
        )

        shutil.copy2(
            os.path.join(SOURCES_PATH, 'file_def_nemo-pisces.xml'),
            os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6', self.job_name, 'PARAM', 'XML')
        )

        shutil.copy2(
            os.path.join(SOURCES_PATH, 'field_def_nemo-pisces.xml'),
            os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6', self.job_name, 'PARAM', 'XML')
        )

        # !!!! SAME as FILE ABOVE file_def_nemo-pisces_offline.xml
        shutil.copy2(
            os.path.join(SOURCES_PATH, 'namelist_pisces_cfg'),
            os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6',
                         self.job_name, 'PARAM', 'NAMELIST')
        )

        shutil.copy2(
            os.path.join(SOURCES_PATH, 'namelist_top_cfg'),
            os.path.join(self.pisces_path, 'modipsl', 'config', 'NEMO_v6',
